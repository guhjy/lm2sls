...
test_that("dfbeta computed correctly", {
...
test_that("cookd computed correctly", {
  expect_equal(cooks.distance(m.2sls), cooks.distance(m.lm))
})

test_that("dffits computed correcxtly", {
  expect_equal(as.vector(dffits(m.2sls)), as.vector(dffits(m.lm)))
})
